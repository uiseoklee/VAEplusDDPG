...
class VAE(nn.Module):
    def __init__(self, latent_dim=23):
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        # 첫 번째 VAE 모델 로드 (이미지1용)
        self.vae1 = VAE(latent_dim=23)
        self.vae1.load_state_dict(torch.load('vae_model1.pth', map_location=torch.device('cpu')), strict=False)
        self.vae1.eval()

        # 두 번째 VAE 모델 로드 (이미지2용)
        self.vae2 = VAE(latent_dim=23)
