...
# 전역 후크 함수 정의
def get_activation_hook(name, activation_sum, activation_count, module, input, output):
    if name not in activation_sum:
        activation_sum[name] = torch.zeros(output.size(1))  # 기본적으로 CPU에 생성됨
        activation_count[name] = 0
    activation_sum[name] += output.mean(dim=0).cpu()  # GPU에서 CPU로 이동
    activation_count[name] += 1
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        # 활성화 값 추적을 위한 딕셔너리 초기화 (누적 합과 카운트 사용)
        self.activation_sum = {}
    ...
    def register_hooks(self):
        # functools.partial을 사용하여 이름과 활성화 딕셔너리를 바인딩
        self.linear2.register_forward_hook(
            functools.partial(get_activation_hook, 'linear2', self.activation_sum, self.activation_count)
        )
        self.linear3.register_forward_hook(
            functools.partial(get_activation_hook, 'linear3', self.activation_sum, self.activation_count)
        )

    def reset_dormant_neurons(self, threshold=1e-3):
        for name in self.activation_sum:
            if self.activation_count[name] == 0:
                continue
            # 평균 활성화 값 계산
            mean_activation = self.activation_sum[name] / self.activation_count[name]
            # dormant neuron 식별
            dormant_neurons = (mean_activation.abs() < threshold)
            if dormant_neurons.any():
                layer = getattr(self, name)
                with torch.no_grad():
                    for idx, is_dormant in enumerate(dormant_neurons):
                        if is_dormant:
                            nn.init.xavier_uniform_(layer.weight[idx:idx+1])
                            if layer.bias is not None:
                                layer.bias[idx:idx+1].zero_()
                print(f"Actor {name}: Reset {dormant_neurons.sum().item()} dormant neurons.")
        # 활성화 값 초기화
        self.activation_sum = {}
        self.activation_count = {}
    ...
    def __getstate__(self):
        ...
        # 후크 제거
        state['_forward_hooks'] = {}
        state['_forward_pre_hooks'] = {}
        return state
...
class Critic(Network):
    ...
    def register_hooks(self):
        # functools.partial을 사용하여 이름과 활성화 딕셔너리를 바인딩
        self.l1.register_forward_hook(
            functools.partial(get_activation_hook, 'l1', self.activation_sum, self.activation_count)
        )
        self.l2.register_forward_hook(
            functools.partial(get_activation_hook, 'l2', self.activation_sum, self.activation_count)
        )
        self.l3.register_forward_hook(
            functools.partial(get_activation_hook, 'l3', self.activation_sum, self.activation_count)
        )
...
class DDPG(OffPolicyAgent):
    def __init__(self, device, sim_speed):
        ...
        # dormant neuron 검사를 위한 카운터 초기화
        self.step_counter = 0
        self.check_interval = 1000  # 1000 스텝마다 검사
    ...
    def train(self, state, action, reward, state_next, done):
        ...
        # 일정 스텝마다 dormant neuron 검사
        if self.step_counter % self.check_interval == 0:
            self.actor.reset_dormant_neurons()
            self.critic.reset_dormant_neurons()
