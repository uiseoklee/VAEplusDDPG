...
# Define the entire VAE class (including encoder and decoder)
# Attention Modules
...
# ConvTranspose2d Block with Attention
...
# VAE Class with Corrected FPN in Decoder
class VAE(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.encoder_blocks = nn.ModuleList([
            Conv2dBlock(1, 32, stride=2),    # Output: (batch_size, 32, 40, 80)
            Conv2dBlock(32, 64, stride=2),   # Output: (batch_size, 64, 20, 40)
            Conv2dBlock(64, 128, stride=2)   # Output: (batch_size, 128, 10, 20)
        ])
        ...
        self.decoder_blocks = nn.ModuleList([
            ConvTranspose2dBlock(128, 128, stride=2),    # Output: (batch_size, 128, 20, 40)
            ConvTranspose2dBlock(128, 64, stride=2),     # Output: (batch_size, 64, 40, 80)
            ConvTranspose2dBlock(64, 32, stride=2),      # Output: (batch_size, 32, 80, 160)
        ])
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.linear2 = nn.Linear(256 + 4, hidden_size)
        self.linear3 = nn.Linear(hidden_size, 256)
        self.linear3_1 = nn.Linear(256, 128)
        self.linear4 = nn.Linear(128, action_size)
    ...
    def forward(self, states, visualize=False):
        ...
        li1_normalized = li1_normalized * darkness_factor
        li2_normalized = li2_normalized * darkness_factor        


        # Extract latent vector through the first VAE encoder
        with torch.no_grad():
            mu1, _ = self.vae1.encode(li1_normalized)
            mu2, _ = self.vae2.encode(li2_normalized)
        ...
        li_s2 = self.dropout(li_s2)
        li_s3 = torch.relu(self.linear3(li_s2))  # (batch_size, hidden_size)
        li_s3 = self.dropout(li_s3)
        li_s3_1 = torch.relu(self.linear3_1(li_s3))  # (batch_size, 128)
        li_s3_1 = self.dropout(li_s3_1)
        action = torch.tanh(self.linear4(li_s3_1))  # (batch_size, action_size)
...
class Critic(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        # Adjust input size to latent vector (46) + s1 (4)
        self.l1 = nn.Linear(256 + 4, int(hidden_size / 2))
