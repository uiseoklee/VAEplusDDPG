...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size, vae1, vae2):
        ...
        self.fc1 = nn.Linear(conv_output_size * 2 + 4, hidden_size // 2)
        ...
        self.se = nn.Sequential(
            nn.Linear(hidden_size // 2, hidden_size // 16),
            nn.ReLU(),
...
class DDPG(OffPolicyAgent):
    def __init__(self, device, sim_speed):
        ...
        # VAE 인스턴스 생성
        self.vae1 = VAE(latent_dim=64).to(self.device)
        self.vae2 = VAE(latent_dim=64).to(self.device)
