...
class Conv1dEncoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.fc = nn.Linear(11520, latent_dim)
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.encoder1 = Conv1dEncoder(latent_dim=46)
        self.encoder2 = Conv2dEncoder(latent_dim=23)
        self.encoder3 = Conv2dEncoder(latent_dim=23)

        # 추가 상태 정보와 결합 후 통과할 완전 연결층 정의
        self.linear2 = nn.Linear(92 + 4, hidden_size)
    ...
    def forward(self, states, visualize=False):
        ...
        if states.dim() == 2:
            ...
            li1 = states[:, :lidar_data_size].view(batch_size, 1, lidar_data_size)  # [batch_size, 1, 360]
            li2 = states[:, lidar_data_size: lidar_data_size + depth_image_size]  # [batch_size, 12800]
            li3 = states[:, lidar_data_size + depth_image_size: 2 * depth_image_size + lidar_data_size]  # [batch_size, 12800]
            s1 = states[:, -4:]  # [batch_size, 4]
        else:
            # Without batch
            li1 = states[:lidar_data_size].view(1, 1, lidar_data_size)  # [1, 1, 360]
            li2 = states[lidar_data_size: lidar_data_size + depth_image_size].view(1, -1)  # [1, 12800]
            li3 = states[lidar_data_size + depth_image_size: 2 * depth_image_size + lidar_data_size].view(1, -1)  # [1, 12800]
            s1 = states[-4:].view(1, -1)  # [1, 4]
        ...
        # 채널 및 이미지 차원으로 변환
        li2_imaged = li2.view(li2.size(0), 1, 80, 160)  # (batch_size, 1, 80, 160)
        
        """
        batch_size = li2.size(0)
        # Convert li1 into a NumPy array if it's a tensor
        if isinstance(li2_imaged, torch.Tensor):
            li2_view = li2_imaged.detach().cpu().numpy()
        ...
        if li2_view.ndim == 4:
            ...
            li2_view = li2_view[0, 0, :, :]

        # Plot the image
        plt.imshow(li2_view, cmap='gray')
        plt.colorbar()
        plt.title('li2_imaged')
        plt.show()
        """

        li3_imaged = li3.view(li3.size(0), 1, 80, 160)

        # Conv2d 인코더를 통해 잠재 벡터 추출
        encoded1 = self.encoder1(li1)
        encoded2 = self.encoder2(li2_imaged)
        encoded3 = self.encoder3(li3_imaged)

        # 잠재 벡터와 추가 상태 정보 결합
        #li_s1 = torch.cat((encoded1, encoded2), dim=1)
        #li_s1 = torch.cat((li_s1, encoded3), dim=1)
        #li_s1 = torch.cat((li_s1, s1), dim=1)

        li_s1 = torch.cat((encoded1, encoded2, encoded3, s1), dim=1)
...
class Critic(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.l1 = nn.Linear(92 + 4, int(hidden_size / 2))
