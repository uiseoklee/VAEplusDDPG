...
class VAE(nn.Module):
    def __init__(self, latent_dim=46):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1),  # 입력 채널 수: 1 (그레이스케일)
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # 80x160 -> 40x80
            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # 40x80 -> 20x40
            nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # 20x40 -> 10x20
            nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1),
            nn.ReLU()
            # MaxPool2d 제거하여 시퀀스 길이 유지 (10x20)
        )
        self.flatten = nn.Flatten()
        self.fc_mu = nn.Linear(256 * 10 * 20, latent_dim)
        self.fc_logvar = nn.Linear(256 * 10 * 20, latent_dim)

        # 디코더 정의
        self.decoder_input = nn.Linear(latent_dim, 256 * 10 * 20)
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(256, 128, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Upsample(scale_factor=2, mode='nearest'),  # 10x20 -> 20x40
            nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Upsample(scale_factor=2, mode='nearest'),  # 20x40 -> 40x80
            nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Upsample(scale_factor=2, mode='nearest'),  # 40x80 -> 80x160
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        # print("actor state_size", self.state_size)

        # VAE 모델 로드
        self.vae = VAE(latent_dim=46)
        self.vae.load_state_dict(torch.load('vae_model_depth80_160_latent46_1000.pth', map_location=torch.device('cpu')), strict=False)
        self.vae.eval()  # 평가 모드로 설정

        # 인코더와 디코더 부분 사용
        self.encoder = self.vae.encoder
        self.fc_mu = self.vae.fc_mu
        self.fc_logvar = self.vae.fc_logvar
        self.decoder = self.vae.decoder

        # 인코더와 디코더의 파라미터를 고정하여 학습되지 않도록 설정
        ...
        for param in self.decoder.parameters():
            param.requires_grad = False  # 디코더가 학습되지 않도록 설정

        # 완전 연결층 정의 (latent_dim + 추가 상태 정보)
    ...
    def forward(self, states, visualize=False):
        if states.dim() == 2:
            # 배치가 있는 경우
            li1 = states[:, :states.shape[1] - 4]  # 이미지 데이터 추출
            s1 = states[:, states.shape[1] - 4:]   # (batch_size, 4)
        else:
            # 배치가 없는 경우
            li1 = states[:states.shape[0] - 4].view(1, -1)
            s1 = states[states.shape[0] - 4:].view(1, -1)
        ...
        # 채널 및 이미지 차원으로 변환
        li1_normalized = li1_normalized.view(li1.size(0), 1, 80, 160)  # (batch_size, 1, 80, 160)

        # VAE 인코더를 통해 잠재 벡터 추출
        with torch.no_grad():
            mu, logvar = self.vae.encode(li1_normalized)
            z = self.vae.reparameterize(mu, logvar)
            x_recon = self.vae.decode(z)  # 재구성된 이미지
        ...
        return action#, x_recon  # 필요에 따라 재구성된 이미지 반환
...
class Critic(Network):
    ...
    def forward(self, states, actions, visualize=False):
        ...
        return x#, x_recon  # 필요에 따라 재구성된 이미지 반환
