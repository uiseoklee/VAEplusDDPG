...
class DDPG(OffPolicyAgent):
    def __init__(self, device, sim_speed):
        ...
        # 
        try:
            self.vae_models = [self.actor.vae1, self.actor.vae2]  # Only two VAE models
            print("VAE models initialized successfully.")
        except AttributeError as e:
            print(f"VAE models initialization failed: {e}")
            self.vae_models = []

        # 
        self.networks = [self.actor, self.critic, self.actor_target, self.critic_target]

    def save_models(self, episode, storage_manager, pickle_data):
        networks = self.networks  # [actor, critic, actor_target, critic_target]
        if not self.vae_models:
            print("VAE models list is empty. Skipping VAE weights saving.")
        storage_manager.save_session(episode, networks, pickle_data, vae_models=self.vae_models)
